...
// a1 is the name of my namspace, he also uses a patern keeps $ asign to jquery and undefined as undefined
...
a1.start = function(hookElementSelection, dataurlForJsonFile) {
	...
	$("#loading").hide().fadeIn("fast",function(){
...
var renderCalculator = function(base) {
	...
	var costHTML = $("<div id='animationbutnon'><button class='btn btn-large btn-success' id='calculateButton'>Calculate Weekly Total</button></div>");
	...
	costHTML.on("click", function(event) {
		...
		costHTML.hide().delay(4000).slideDown("slow");


		resultsHTML.hide().delay(2000).slideDown("slow");
...
var splitTotalsAndPrint = function(base, ingredientTotals)
{
	...
	for (supplierName in supplierTotals) {
		...
		var finalHTML = $("<div class='row'></div>");
...
a1.Product = function(name,cost,per,supplier) {
	...
	this.render = function(base, amount) {
		...
		var finalHTML = $("<div class='col-md-1'></div>");
...
a1.Recipe = function(name, ingredients ) {
	...
	this.render = function(base) {
		...
		var finalHTML = $("<div class='recipe' id='recipeList'></div><br/>");
		...
		finalHTML.hide().delay(4000).slideDown("slow");
...
a1.Supplier = function(name) {
	...
	this.render = function(base) {
		...
		var finalHTML = $("<div class='supplier'></div>");
		finalHTML.append(nameHTML);
		base.append(finalHTML);
	};
};
